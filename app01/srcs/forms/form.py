...
class UserModelForm(BootstrapModelForm):
    ...
    class Meta:  # 这里必须是Meta四个字母，不能错
        ...
        fields = ["user_id", "name", "password", "account", "age", "creat_time", "gender","email"]  # 这里必须是fields字母，不能错
        # widgets = {
...
class NumModelForm(BootstrapModelForm):
    class Meta:
        model = Schedule  # 这里必须是model字母，不能错
    ...
    def clean_mobile(self):
        ...
        if Schedule.objects.filter(mobile=txt_mobile).exists():
...
class NumModelFormEdit(BootstrapModelForm):
    ...
    def clean_mobile(self):
        ...
        if len(txt_mobile) != 11:
            raise ValidationError("格式错误")
        # 编辑时排除自身
        if Schedule.objects.filter(mobile=txt_mobile).exclude(id=id).exists():
            raise ValidationError("手机号已经存在")
        return txt_mobile
...
class ScheduleModelForm(forms.ModelForm):
    class Meta:
        ...
        fields = "__all__"
        widgets = {
            "play_time": forms.DateTimeInput(attrs={"type": "datetime-local"})
        }

class ScheduleModelFormEdit(forms.ModelForm):
    class Meta:
        model = Schedule
        fields = "__all__"
        widgets = {
